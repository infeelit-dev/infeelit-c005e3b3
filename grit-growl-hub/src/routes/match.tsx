...
function MatchPage() {
  ...
  const loadMatches = useCallback(async () => {
    ...
    const userData = (userRes?.data || userRes?.attendee) as AttendeeRow | null;
    ...
    // Fallback to legacy get-active if new action not deployed yet
    ...
    const results = await findTopMatches(toOracleProfile(userData), candidates.map(toOracleProfile));
    ...
    // Mark that this user has been shown their evening matches
        ...
        <div style={{ maxWidth: "420px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "14px", color: "#666", lineHeight: 1.6, marginBottom: "24px" }}>
            {peopleCount > 0
              ...
              : "Your matches will appear as more people complete onboarding."}
          </p>
          <button
            onClick={() => loadMatches()}
            style={{
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "32px" }}>
        ...
        <p style={{ fontSize: "22px", fontWeight: 600, color: "#fff" }}>
          {matches.length} people waiting for you
        </p>
      </div>

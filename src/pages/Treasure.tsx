...
const Player = ({
  ...
  useEffect(() => {
    if (!isDemo) startCinemaRitual();
    else setCinemaPhase("playing");
    return () => {
      timersRef.current.forEach((t) => clearTimeout(t));
    };
  }, [memory.id]);
  ...
  const handleMediaEnded = () => {
    ...
    const mirror =
      lang === "fr"
        ? `Et toi ${name}, qu'as-tu ressenti en écoutant "${memory.title}" ?`
        : lang === "ar"
          ? `وأنت ${name}، ماذا شعرت وأنت تستمع إلى "${memory.title}" ؟`
          : `And you ${name}, what did you feel listening to "${memory.title}"?`;
          ...
          {cinemaPhase !== "playing" && (
            <div
              ...
              style={{
                ...
                backdropFilter:
                  cinemaPhase === "blur" ? "blur(8px)" : cinemaPhase === "text" ? "blur(4px)" : "blur(0px)",
                backgroundColor:
                  cinemaPhase === "blur"
                    ? "rgba(0,0,0,0.65)"
                    : cinemaPhase === "text"
            ...
            >
              ...
              >
                {lang === "fr" ? "Appuie pour passer" : lang === "ar" ? "اضغط للتخطي" : "Tap to skip"}
              </p>
            </div>
          )}
          {showMirror && (
            <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center gap-6 px-6 z-20">
              ...
              <button onClick={() => setShowMirror(false)} className="text-white/30 text-xs">

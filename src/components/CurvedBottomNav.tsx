...
const CurvedBottomNav = ({ onPlusClick, circleBadge = 0, familySpace = false }: CurvedBottomNavProps) => {
  ...
  const NAV_ITEMS = [
    ...
    {
      icon: MessageCircle,
      label: pickLocalized(lang, { ar: "همسات", fr: "Murmures", en: "Whispers" }),
      path: "/whispers",
    },
    {
      icon: "flame",
    ...
    <div className="absolute bottom-0 left-0 right-0 z-20">
      ...
      <style
        dangerouslySetInnerHTML={{
          __html:
            "@keyframes flameAlive { 0%, 100% { transform: scale(1); filter: drop-shadow(0 0 4px rgba(232,116,42,0.6)); } 50% { transform: scale(1.08); filter: drop-shadow(0 0 8px rgba(232,116,42,0.9)); } }",
        }}
      />
    </div>
  );

...
const SparkBubble = ({ forceOpen, onSparkClose }: SparkBubbleProps) => {
  ...
  return (
    <>
      ...
      {!expanded && !forceOpen && (
        <button onClick={handleBubbleClick} className={`absolute z-[15] cursor-pointer transition-transform active:scale-90 bg-transparent border-none p-0 ${triggerApproach ? "gentle-approach" : ""}`} style={{ left: `${position.x}%`, top: `${position.y}%`, width: "90px", height: "90px", transition: "left 8s ease-in-out, top 8s ease-in-out", transform: "translate(-50%,-50%)" }}>
          {[...Array(10)].map((_, i) => (
          ...
          <div className="heartbeat-pulse absolute rounded-full" style={{ width: "90px", height: "90px", background: "radial-gradient(circle at 45% 45%, rgba(255,220,80,0.95) 0%, rgba(255,180,40,0.85) 25%, rgba(232,116,42,0.5) 55%, rgba(180,70,10,0.15) 100%)", border: "2px solid rgba(255,200,60,0.7)" }}>
            ...
            {showBadge && (
              <div className="absolute -top-2 -left-2 px-1.5 py-0.5 rounded-full flex items-center justify-center" style={{ background: "rgba(232,116,42,0.9)", boxShadow: "0 0 6px rgba(232,116,42,0.4)" }}>
          ...
          <div className="expand-in w-full max-w-md mx-6 rounded-3xl px-6 py-8 text-center relative" style={{ background: "#FFF9F2", border: "1px solid rgba(212,168,83,0.2)", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }} onClick={(e) => e.stopPropagation()}>
            ...
            {showNameInput ? (
              <form onSubmit={handleNameSubmit} className="flex flex-col items-center gap-5">
                <p className="text-[#E8742A] text-[10px] font-black uppercase tracking-[0.3em]">{lang === "ar" ? "ما اسمك ؟" : lang === "fr" ? "Comment tu t'appelles ?" : "What's your name?"}</p>
                <div className="relative w-full max-w-[260px] rounded-full px-6 py-4 transition-all border bg-[#FFFFFF] border-[#D4A853]/30 focus-within:border-[#E8742A] focus-within:shadow-[0_0_20px_rgba(232,116,42,0.1)]"><input type="text" value={nameInput} onChange={(e) => setNameInput(e.target.value)} placeholder={lang === "ar" ? "اسمك الأول" : lang === "fr" ? "Ton prénom" : "Your first name"} className="w-full bg-transparent outline-none text-[#3D2B1F] text-lg text-center font-serif" autoFocus /></div>
                <button type="submit" disabled={nameInput.trim().length < 1} className="w-full max-w-[260px] py-4 rounded-full font-bold text-base transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed border-none cursor-pointer" style={{ background: "linear-gradient(135deg, #E8742A, #D4621A)", color: "#fff", boxShadow: "0 4px 20px rgba(232,116,42,0.3)" }}>{lang === "ar" ? "اكتشف قصصك" : lang === "fr" ? "Découvre tes histoires" : "Discover your stories"}</button>
              </form>
            ) : (
              <>
                <p className="text-[#E8742A] text-[10px] font-black uppercase tracking-[0.3em] mb-1">{lang === "ar" ? "اختر قصتك" : lang === "fr" ? "Choisis ton histoire" : "Choose your story"}</p>
                <p style={{ color: "#E8742A", fontSize: "13px", fontStyle: "italic", fontFamily: "Georgia, serif", marginBottom: "16px" }}>{lang === "ar" ? (userName ? `${userName}، احكِ لهم.` : "احكِ لهم.") : lang === "fr" ? (userName ? `${userName}, raconte-leur.` : "Raconte-leur.") : (userName ? `${userName}, tell them.` : "Tell them.")}</p>
                ...
                <div className="flex justify-center gap-2 mt-2 mb-4">{[0, 1, 2].map((i) => (<div key={i} className="w-2 h-2 rounded-full transition-all duration-300" style={{ background: selectedCard === i ? "#E8742A" : "rgba(61,43,31,0.15)", transform: selectedCard === i ? "scale(1.3)" : "scale(1)" }} />))}</div>
                {showButton && (<button onClick={handleRecord} className="w-full py-4 rounded-full font-bold text-base transition-all hover:scale-[1.02] active:scale-[0.98] border-none cursor-pointer" style={{ background: "linear-gradient(135deg, #E8742A, #D4621A)", color: "#fff", boxShadow: "0 4px 20px rgba(232,116,42,0.3)" }}>{getButtonText()}</button>)}
                {!showButton && (<div className="flex justify-center"><div className="w-8 h-8 border-2 border-[#E8742A]/20 border-t-[#E8742A] rounded-full animate-spin" /></div>)}
                {sparkBalance > 0 && (<p className="text-[#3D2B1F]/30 text-[9px] mt-3">{lang === "ar" ? `لديك ${sparkBalance} شرارات` : lang === "fr" ? `Tu as ${sparkBalance} étincelles` : `You have ${sparkBalance} sparks`}</p>)}
              </>
            )}
          </div>
        </div>

    ...
    <>
      ...
      {!expanded && !forceOpen && (
        <button onClick={handleBubbleClick} className="absolute z-[15] cursor-pointer transition-transform active:scale-90 bg-transparent border-none p-0" style={{ left: `${position.x}%`, top: `${position.y}%`, width: "90px", height: "90px", transition: "left 8s ease-in-out, top 8s ease-in-out", transform: "translate(-50%,-50%)" }}>
          {[...Array(10)].map((_, i) => (
            <div key={i} style={{ position: "absolute", top: "50%", left: "50%", width: i % 3 === 0 ? "4px" : "2.5px", height: i % 3 === 0 ? "4px" : "2.5px", borderRadius: "50%", background: `hsl(${35 + i * 4},90%,${60 + i * 2}%)`, boxShadow: "0 0 5px rgba(255,200,60,0.9)", animation: `particleUp ${2.5 + Math.random() * 2}s ease-out infinite ${i * 0.35}s`, "--dx": `${(Math.random() - 0.5) * 40}px` } as React.CSSProperties} />
          ))}
            ...
            {showNameInput ? (
              <form onSubmit={handleNameSubmit} className="flex flex-col items-center gap-5">
                ...
                <div className="relative w-full max-w-[260px] rounded-full px-6 py-4 transition-all border bg-[#FFFFFF] border-[#D4A853]/30 focus-within:border-[#E8742A] focus-within:shadow-[0_0_20px_rgba(232,116,42,0.1)]">
                  <input type="text" value={nameInput} onChange={(e) => setNameInput(e.target.value)} placeholder={lang === "ar" ? "اسمك الأول" : lang === "fr" ? "Ton prénom" : "Your first name"} className="w-full bg-transparent outline-none text-[#3D2B1F] text-lg text-center font-serif" autoFocus />
                </div>
              ...
              <>
                ...
                <p style={{ color: "#E8742A", fontSize: "13px", fontStyle: "italic", fontFamily: "Georgia, serif", marginBottom: "16px" }}>
                ...
                <div className="flex justify-center gap-2 mt-2 mb-4">
                  {[0, 1, 2].map((i) => ( <div key={i} className="w-2 h-2 rounded-full transition-all duration-300" style={{ background: selectedCard === i ? "#E8742A" : "rgba(61,43,31,0.15)", transform: selectedCard === i ? "scale(1.3)" : "scale(1)" }} /> ))}
                </div>
                {showButton && (
                ...
                {sparkBalance > 0 && (
                  <p className="text-[#3D2B1F]/30 text-[9px] mt-3">{lang === "ar" ? `لديك ${sparkBalance} شرارات` : lang === "fr" ? `Tu as ${sparkBalance} étincelles` : `You have ${sparkBalance} sparks`}</p>
                )}
              </>
            )}
          </div>
